...
Server_GLOBLA = io.popen("echo $SSH_CLIENT | awk '{ print $1}'"):read('*a')
------------------------------------------------------------------------------------------------------------
local function Load_File()
local f = io.open("./Info.lua", "r")  
if not f then   
if not redis:get(Server_GLOBLA.."Token_DevGLOBLA") then
...
redis:set(Server_GLOBLA.."Token_DevGLOBLA",token)
end 
else
...
do 
local GLOBLA_INFO = {
Id_DevGLOBLA = ]]..redis:get(Server_GLOBLA.."Id_DevGLOBLA")..[[,
UserName_GLOBLA = "]]..redis:get(Server_GLOBLA.."User_DevGLOBLA1")..[[",
Token_Bot = "]]..redis:get(Server_GLOBLA.."Token_DevGLOBLA")..[["
}
return GLOBLA_INFO
end
...
#!/usr/bin/env bash
cd $HOME/GLOBLA
token="]]..redis:get(Server_GLOBLA.."Token_DevGLOBLA")..[["
while(true) do
rm -fr ../.telegram-cli
...
f:close()  
redis:del(Server_GLOBLA.."Token_DevGLOBLA");redis:del(Server_GLOBLA.."Id_DevGLOBLA");redis:del(Server_GLOBLA.."User_DevGLOBLA1")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_GLOBLA
bot_id = token:match("(%d+)")  
Id_Dev = sudos.Id_DevGLOBLA
Ids_Dev = {sudos.Id_DevGLOBLA,bot_id}
...
function Dev_GLOBLA(msg)  
    ...
    if hash or Dev_GLOBLA(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Bot(msg) then     
    return true    
    else    
    return false    
    end 
    ...
    if hash or Dev_GLOBLA(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Bot(msg) then     
    return true    
    else    
    return false    
    end 
    ...
    if hash or Dev_GLOBLA(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Bot(msg) then     
    return true    
    else    
    return false    
    end 
    ...
    if hash or Dev_GLOBLA(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Admin(msg) or Bot(msg) then     
    return true 
    else 
    return false 
    end 
...
var = "البوت"
elseif redis:sismember('GLOBLA:'..bot_id.."Developer:Bot", user_id) then
var = redis:get('GLOBLA:'..bot_id.."Developer:Bot:Reply"..chat_id) or redis:get('GLOBLA:'..bot_id.."Add:Validity:Users"..chat_id..user_id) or "المطور"  
elseif redis:sismember('GLOBLA:'..bot_id.."President:User"..chat_id, user_id) then
var = redis:get('GLOBLA:'..bot_id.."President:User:Reply"..chat_id) or redis:get('GLOBLA:'..bot_id.."Add:Validity:Users"..chat_id..user_id) or "المالك"
elseif redis:sismember('GLOBLA:'..bot_id.."Basic:User"..chat_id, user_id) then
var = redis:get('GLOBLA:'..bot_id.."Basic:User:Reply"..chat_id) or redis:get('GLOBLA:'..bot_id.."Add:Validity:Users"..chat_id..user_id) or "المنشئ الاساسي"
elseif redis:sismember('GLOBLA:'..bot_id..'Constructor:Group'..chat_id, user_id) then
var = redis:get('GLOBLA:'..bot_id.."Constructor:Group:Reply"..chat_id) or redis:get('GLOBLA:'..bot_id.."Add:Validity:Users"..chat_id..user_id) or "المنشئ"  
elseif redis:sismember('GLOBLA:'..bot_id..'Manager:Group'..chat_id, user_id) then
var = redis:get('GLOBLA:'..bot_id.."Manager:Group:Reply"..chat_id) or redis:get('GLOBLA:'..bot_id.."Add:Validity:Users"..chat_id..user_id) or "المدير"  
elseif redis:sismember('GLOBLA:'..bot_id..'Admin:Group'..chat_id, user_id) then
var = redis:get('GLOBLA:'..bot_id.."Admin:Group:Reply"..chat_id) or redis:get('GLOBLA:'..bot_id.."Add:Validity:Users"..chat_id..user_id) or "الادمن"  
elseif redis:sismember('GLOBLA:'..bot_id..'Vip:Group'..chat_id, user_id) then  
var = redis:get('GLOBLA:'..bot_id.."Vip:Group:Reply"..chat_id) or redis:get('GLOBLA:'..bot_id.."Add:Validity:Users"..chat_id..user_id) or "المميز"  
else  
var = redis:get('GLOBLA:'..bot_id.."Mempar:Group:Reply"..chat_id) or redis:get('GLOBLA:'..bot_id.."Add:Validity:Users"..chat_id..user_id) or "العضو"
end  
return var
end 
------------------------------------------------------------------------------------------------------------
...
var_Api = "https://api.telegram.org/bot"..token 
local Url_Api = var_Api.."/sendMessage?chat_id=" .. chat_id .. "&text=" .. URL.escape(text) 
...
local var_Api = "https://api.telegram.org/bot"..token.."/sendMessage?chat_id="..chat_id.."&text="..URL.escape(text).."&parse_mode=Markdown&disable_web_page_preview=true&reply_markup="..URL.escape(JSON.encode(response)) 
if reply_id then 
var_Api = var_Api.."&reply_to_message_id="..reply_id 
end 
return Get_Api(var_Api) 
end
------------------------------------------------------------------------------------------------------------
function GetInputFile(file)  
local file = file or ""   
...
pcall(tdcli_function ({
ID = "ChangeChatMembervar",
chat_id_ = chat,
user_id_ = user,
var_ = {ID = "ChatMembervarKicked"},},function(arg,data) end,nil))
end
------------------------------------------------------------------------------------------------------------
function Send_Options(msg,user_id,var,text)
...
if var == "Close_var" then
send(msg.chat_id_, msg.id_,"⌔︙بواسطه -› ["..data.first_name_.."](T.me/"..UserName..")".."\n"..text.."")
return false
end
if var == "Close_var_Ktm" then
send(msg.chat_id_, msg.id_,"⌔︙بواسطه -› ["..data.first_name_.."](T.me/"..UserName..")".."\n"..text.."\n⌔︙خاصية - الكتم 𓂅 .\n")
return false
end
if var == "Close_var_Kick" then
send(msg.chat_id_, msg.id_,"⌔︙بواسطه -› ["..data.first_name_.."](T.me/"..UserName..")".."\n"..text.."\n⌔︙خاصية - الطرد 𓂅 .\n")
return false
end
if var == "Close_var_Kid" then
send(msg.chat_id_, msg.id_,"⌔︙بواسطه -› ["..data.first_name_.."](T.me/"..UserName..")".."\n"..text.."\n⌔︙خاصية - التقييد 𓂅 .\n")
return false
end
if var == "Open_var" then
send(msg.chat_id_, msg.id_,"⌔︙بواسطه -› ["..data.first_name_.."](T.me/"..UserName..")".."\n"..text)
return false
end
if var == "reply" then
...
function Send_Optionspv(chat,idmsg,user_id,var,text)
tdcli_function ({ID = "GetUser",user_id_ = user_id},function(arg,data) 
if data.first_name_ ~= false then
local UserName = (data.username_ or "Spirta")
for gmatch in string.gmatch(data.first_name_, "[^%s]+") do
...
redis:set('GLOBLA:'..bot_id.."Lock:tagservrbot"..Id_Group,true)   
list ={"Lock:Bot:kick","Lock:User:Name","Lock:hashtak","Lock:Cmd","Lock:Link","Lock:forward","Lock:Keyboard","Lock:geam","Lock:Photo","Lock:Animation","Lock:Video","Lock:Audio","Lock:vico","Lock:Sticker","Lock:Document","Lock:Unsupported","Lock:Markdaun","Lock:Contact","var:Lock:Spam"}
...
if Info_Group.var_Dev then
if Info_Group.var_Dev ~= "" then
redis:set('GLOBLA:'..bot_id.."Developer:Bot:Reply"..Id_Group,Info_Group.var_Dev)   
end
end
if Info_Group.var_Prt then
if Info_Group.var_Prt ~= "" then
redis:set('GLOBLA:'..bot_id.."President:User:Reply"..Id_Group,Info_Group.var_Prt)   
end
end
if Info_Group.var_Prt then
if Info_Group.var_Prt ~= "" then
redis:set('GLOBLA:'..bot_id.."Basic:User:Reply"..Id_Group,Info_Group.var_Prt)   
end
end
if Info_Group.var_Cto then
if Info_Group.var_Cto ~= "" then
redis:set('GLOBLA:'..bot_id.."Constructor:Group:Reply"..Id_Group,Info_Group.var_Cto)   
end
end
if Info_Group.var_Own then
if Info_Group.var_Own ~= "" then
redis:set('GLOBLA:'..bot_id.."Manager:Group:Reply"..Id_Group,Info_Group.var_Own)   
end
end
if Info_Group.var_Md then
if Info_Group.var_Md ~= "" then
redis:set('GLOBLA:'..bot_id.."Admin:Group:Reply"..Id_Group,Info_Group.var_Md)   
end
end
if Info_Group.var_Vip then
if Info_Group.var_Vip ~= "" then
redis:set('GLOBLA:'..bot_id.."Vip:Group:Reply"..Id_Group,Info_Group.var_Vip)   
end
end
if Info_Group.var_Mem then
if Info_Group.var_Mem ~= "" then
redis:set('GLOBLA:'..bot_id.."Mempar:Group:Reply"..Id_Group,Info_Group.var_Mem)   
end
end
if Info_Group.LinkGroup then
if Info_Group.LinkGroup ~= "" then
redis:set('GLOBLA:'..bot_id.."var:link:set:Group"..Id_Group,Info_Group.LinkGroup)   
end
end
end
...
deved = false
local var = redis:sismember('GLOBLA:'..bot_id.."Developer:Bot", msg.sender_user_id_) 
if var then
deved = true  
end
if Dev_GLOBLA(msg) == true then  
deved = true  
end  
return deved
end
...
sens = 400  
if redis:get('GLOBLA:'..bot_id.."Lock:Spam"..msg.chat_id_) == "del" and not Vips(msg) and string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get('GLOBLA:'..bot_id.."Lock:Spam"..msg.chat_id_) == "ked" and not Vips(msg) and string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
RestrictChat(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get('GLOBLA:'..bot_id.."Lock:Spam"..msg.chat_id_) == "kick" and not Vips(msg) and string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
KickGroup(msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
elseif redis:get('GLOBLA:'..bot_id.."Lock:Spam"..msg.chat_id_) == "ktm" and not Vips(msg) and string.len(msg.content_.text_) > (sens) or ctrl_ > (sens) or real_ > (sens) then 
redis:sadd('GLOBLA:'..bot_id.."Silence:User:Group"..msg.chat_id_,msg.sender_user_id_)
Delete_Message(msg.chat_id_,{[0] = msg.id_}) 
end
end
if text and redis:get('GLOBLA:'..bot_id..'Lock:Fsh'..msg.chat_id_) and not Vips(msg) then
...
if text and redis:get('GLOBLA:'..bot_id..'Lock:Far'..msg.chat_id_) and not Vips(msg) then
...
send(msg.chat_id_, msg.id_,'اهلا مطوري')
return false 
end
if redis:get('GLOBLA:'..bot_id.."Lock:kanser"..msg.chat_id_) then
...
local var_welcome = redis:get('GLOBLA:'..bot_id.."Chek:Welcome"..msg.chat_id_)
if var_welcome and not redis:get('GLOBLA:'..bot_id.."Lock:tagservr"..msg.chat_id_) then
...
if Msg_Pin and redis:get('GLOBLA:'..bot_id.."var:lockpin"..msg.chat_id_) then
...
--------------------------------------------------------------------------------------------------------------
if redis:get('GLOBLA:'..bot_id.."var:link:set"..msg.chat_id_..""..msg.sender_user_id_) then
if text == "الغاء" then
send(msg.chat_id_,msg.id_, "\n⌔︙تم الغاء امر حفظ الرابط") 
redis:del('GLOBLA:'..bot_id.."var:link:set"..msg.chat_id_..""..msg.sender_user_id_) 
return false
end
if text and text:match("(https://telegram.me/joinchat/%S+)") or text and text:match("(https://t.me/joinchat/%S+)") then     
local Link = text:match("(https://telegram.me/joinchat/%S+)") or text:match("(https://t.me/joinchat/%S+)")   
redis:set('GLOBLA:'..bot_id.."var:link:set:Group"..msg.chat_id_,Link)
...
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get('GLOBLA:'..bot_id.."var:Random:Sm"..msg.chat_id_) or "").."" and not redis:get('GLOBLA:'..bot_id.."var:Set:Sma"..msg.chat_id_) then
if not redis:get('GLOBLA:'..bot_id.."var:Set:Sma"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,"\n⌔︙لقد فزت في اللعبه \n⌔︙اللعب مره اخره وارسل - سمايل او سمايلات")
redis:incrby('GLOBLA:'..bot_id.."Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set('GLOBLA:'..bot_id.."var:Set:Sma"..msg.chat_id_,true)
return false
end 
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get('GLOBLA:'..bot_id.."var:Klam:Speed"..msg.chat_id_) or "").."" and not redis:get('GLOBLA:'..bot_id.."var:Speed:Tr"..msg.chat_id_) then
if not redis:get('GLOBLA:'..bot_id.."var:Speed:Tr"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,"\n⌔︙لقد فزت في اللعبه \n⌔︙اللعب مره اخره وارسل - الاسرع او ترتيب")
redis:incrby('GLOBLA:'..bot_id.."Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set('GLOBLA:'..bot_id.."var:Speed:Tr"..msg.chat_id_,true)
end 
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get('GLOBLA:'..bot_id.."var:Klam:Hzor"..msg.chat_id_) or "").."" and not redis:get('GLOBLA:'..bot_id.."var:Set:Hzora"..msg.chat_id_) then
if not redis:get('GLOBLA:'..bot_id.."var:Set:Hzora"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,"\n⌔︙لقد فزت في اللعبه \n⌔︙اللعب مره اخره وارسل - حزوره")
redis:incrby('GLOBLA:'..bot_id.."Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set('GLOBLA:'..bot_id.."var:Set:Hzora"..msg.chat_id_,true)
end 
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get('GLOBLA:'..bot_id.."var:Maany"..msg.chat_id_) or "").."" and not redis:get('GLOBLA:'..bot_id.."var:Set:Maany"..msg.chat_id_) then
if not redis:get('GLOBLA:'..bot_id.."var:Set:Maany"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,"\n⌔︙لقد فزت في اللعبه \n⌔︙اللعب مره اخره وارسل - معاني")
redis:incrby('GLOBLA:'..bot_id.."Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set('GLOBLA:'..bot_id.."var:Set:Maany"..msg.chat_id_,true)
end 
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get('GLOBLA:'..bot_id.."var:Set:Aks:Game"..msg.chat_id_) or "").."" and not redis:get('GLOBLA:'..bot_id.."var:Set:Aks"..msg.chat_id_) then
if not redis:get('GLOBLA:'..bot_id.."var:Set:Aks"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,"\n⌔︙لقد فزت في اللعبه \n⌔︙اللعب مره اخره وارسل - العكس")
redis:incrby('GLOBLA:'..bot_id.."Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set('GLOBLA:'..bot_id.."var:Set:Aks"..msg.chat_id_,true)
end 
------------------------------------------------------------------------------------------------------------
if redis:get('GLOBLA:'..bot_id.."var:GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 20 then
send(msg.chat_id_, msg.id_,"⌔︙عذرآ لا يمكنك تخمين عدد اكبر من ال { 20 } خمن رقم ما بين ال{ 1 و 20 }\n")
return false  end 
local GETNUM = redis:get('GLOBLA:'..bot_id.."var:GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
redis:del('GLOBLA:'..bot_id.."var:SADD:NUM"..msg.chat_id_..msg.sender_user_id_)
redis:del('GLOBLA:'..bot_id.."var:GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
redis:incrby('GLOBLA:'..bot_id.."var:SADD:NUM"..msg.chat_id_..msg.sender_user_id_,1)
if tonumber(redis:get('GLOBLA:'..bot_id.."var:SADD:NUM"..msg.chat_id_..msg.sender_user_id_)) >= 3 then
redis:del('GLOBLA:'..bot_id.."var:SADD:NUM"..msg.chat_id_..msg.sender_user_id_)
redis:del('GLOBLA:'..bot_id.."var:GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
------------------------------------------------------------------------------------------------------------
if redis:get('GLOBLA:'..bot_id.."var:SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 6 then
send(msg.chat_id_, msg.id_,"⌔︙عذرا لا يوجد سواء { 6 } اختيارات فقط ارسل اختيارك مره اخرى\n")
return false  end 
local GETNUM = redis:get('GLOBLA:'..bot_id.."var:Games:Bat"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
redis:del('GLOBLA:'..bot_id.."var:SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get('GLOBLA:'..bot_id.."var::Set:Moktlf"..msg.chat_id_) or "").."" then 
if not redis:get('GLOBLA:'..bot_id.."var:Set:Moktlf:Bot"..msg.chat_id_) then 
redis:del('GLOBLA:'..bot_id.."var::Set:Moktlf"..msg.chat_id_)
send(msg.chat_id_, msg.id_,"\n⌔︙لقد فزت في اللعبه \n⌔︙اللعب مره اخره وارسل - المختلف")
redis:incrby('GLOBLA:'..bot_id.."Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set('GLOBLA:'..bot_id.."var:Set:Moktlf:Bot"..msg.chat_id_,true)
end
------------------------------------------------------------------------------------------------------------
if text == ""..(redis:get('GLOBLA:'..bot_id.."var:Set:Amth"..msg.chat_id_) or "").."" then 
if not redis:get('GLOBLA:'..bot_id.."var:Set:Amth:Bot"..msg.chat_id_) then 
redis:del('GLOBLA:'..bot_id.."var:Set:Amth"..msg.chat_id_)
send(msg.chat_id_, msg.id_,"\n⌔︙لقد فزت في اللعبه \n⌔︙اللعب مره اخره وارسل - امثله")
redis:incrby('GLOBLA:'..bot_id.."Num:Add:Games"..msg.chat_id_..msg.sender_user_id_, 1)  
end
redis:set('GLOBLA:'..bot_id.."var:Set:Amth:Bot"..msg.chat_id_,true)
end
------------------------------------------------------------------------------------------------------------
if redis:get('GLOBLA:'..bot_id.."var:Add:msg:user" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
redis:del('GLOBLA:'..bot_id.."id:user"..msg.chat_id_)  
send(msg.chat_id_,msg.id_, "\n⌔︙تم الغاء امر اضافة رسائل") 
redis:del('GLOBLA:'..bot_id.."var:Add:msg:user" .. msg.chat_id_ .. "" .. msg.sender_user_id_)  
return false  
end 
redis:del('GLOBLA:'..bot_id.."var:Add:msg:user" .. msg.chat_id_ .. "" .. msg.sender_user_id_)  
...
------------------------------------------------------------------------------------------------------------
if redis:get('GLOBLA:'..bot_id.."var:games:add" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
redis:del('GLOBLA:'..bot_id.."idgem:user"..msg.chat_id_)  
send(msg.chat_id_,msg.id_, "\n⌔︙تم الغاء امر اضافة جواهر") 
redis:del('GLOBLA:'..bot_id.."var:games:add" .. msg.chat_id_ .. "" .. msg.sender_user_id_)  
return false  
end 
redis:del('GLOBLA:'..bot_id.."var:games:add" .. msg.chat_id_ .. "" .. msg.sender_user_id_)  
local numadded = string.match(text, "(%d+)") 
...
redis:set('GLOBLA:'..bot_id.."Filter:Reply:var"..msg.sender_user_id_..msg.chat_id_, text)  
...
redis:del('GLOBLA:'..bot_id.."Filter:Reply1"..msg.sender_user_id_..msg.chat_id_)  
local test = redis:get('GLOBLA:'..bot_id.."Filter:Reply:var"..msg.sender_user_id_..msg.chat_id_)  
if text then   
redis:set('GLOBLA:'..bot_id.."Filter:Reply2"..test..msg.chat_id_, text)  
end  
redis:del('GLOBLA:'..bot_id.."Filter:Reply:var"..msg.sender_user_id_..msg.chat_id_)  
return false  end  
end
------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------------------
if text and not redis:get('GLOBLA:'..bot_id.."var:Reply:Manager"..msg.chat_id_) then
...
local var_Gps = Get_Rank(msg.sender_user_id_,msg.chat_id_)
...
local Text = Text:gsub('#stast',var_Gps)
send(msg.chat_id_, msg.id_,'['..Text..']')
...
redis:set('GLOBLA:'..bot_id.."Lock:tagservrbot"..Groups[i],true)   
list ={"Lock:Bot:kick","Lock:User:Name","Lock:hashtak","Lock:Cmd","Lock:Link","Lock:forward","Lock:Keyboard","Lock:geam","Lock:Photo","Lock:Animation","Lock:Video","Lock:Audio","Lock:vico","Lock:Sticker","Lock:Document","Lock:Unsupported","Lock:Markdaun","Lock:Contact","var:Lock:Spam"}
...
------------------------------------------------------------------------------------------------------------
if text and not redis:get('GLOBLA:'..bot_id.."var:Reply:Sudo"..msg.chat_id_) then
if not redis:sismember('GLOBLA:'..bot_id..'Spam_For_Bot'..msg.sender_user_id_,text) then
...
if not Dev_GLOBLA(msg) and not redis:sismember('GLOBLA:'..bot_id..'User:Ban:Pv',msg.sender_user_id_) and not redis:get('GLOBLA:'..bot_id..'Lock:Twasl') then
send(msg.sender_user_id_,msg.id_,'⌔︙تم ارسال رسالتك الى المطور ← { ['..SudoUserName..'] }')    
local List_id = {Developer,msg.sender_user_id_}
for k,v in pairs(List_id) do   
tdcli_function({ID="GetChat",chat_id_=v},function(arg,chat) end,nil)
end
tdcli_function({ID="ForwardMessages",chat_id_=Developer,from_chat_id_= msg.sender_user_id_,message_ids_={[0]=msg.id_},disable_notification_=1,from_background_=1},function(arg,data) 
if data and data.messages_ and data.messages_[0] ~= false and data.ID ~= "Error" then
if data and data.messages_ and data.messages_[0].content_.sticker_ then
Send_Optionspv(Developer,0,msg.sender_user_id_,"reply_Pv","⌔︙قام بارسال الملصق")  
return false
end
end
end,nil)
...
if text == 'حظر' then
Send_Optionspv(Developer,msg.id_,UserForward,"reply_Pv","⌔︙تم حظره من تواصل البوت")  
redis:sadd('GLOBLA:'..bot_id..'User:Ban:Pv',data.id_)  
return false  
elseif text =='الغاء الحظر' then
Send_Optionspv(Developer,msg.id_,UserForward,"reply_Pv","⌔︙تم الغاء حظره من تواصل البوت")  
redis:srem('GLOBLA:'..bot_id..'User:Ban:Pv',data.id_)  
return false  
end 
tdcli_function({ID='GetChat',chat_id_=UserForward},function(a,s) end,nil)
tdcli_function({ID="SendChatAction",chat_id_=UserForward,action_={ID="SendMessageTypingAction",progress_=100}},function(arg,Get_var) 
if (Get_var.code_) == (400) or (Get_var.code_) == (5) then
Send_Optionspv(Developer,msg.id_,UserForward,"reply_Pv","⌔︙قام بحظر البوت لا تستطيع ارسال له رسائل")  
return false  
end 
if text then    
...
Send_Optionspv(Developer,msg.id_,UserForward,"reply_Pv","⌔︙تم ارسال رسالتك اليه بنجاح")  
end,nil)end,nil)
end
...
function Functionvar(arg, result)
if (result.id_) then
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^حظر عام @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^الغاء العام @(.*)$") }, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^اضف مطور @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^حذف مطور @(.*)$")}, Functionvar, nil)
...
if data and data.type_ and data.type_.channel_ and data.type_.channel_.var_ and data.type_.channel_.var_.ID == "ChatMembervarMember" then
redis:srem('GLOBLA:'..bot_id..'Groups_Users',group[i])  
w = w + 1
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.var_ and data.type_.channel_.var_.ID == "ChatMembervarLeft" then
redis:srem('GLOBLA:'..bot_id..'Groups_Users',group[i])  
q = q + 1
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.var_ and data.type_.channel_.var_.ID == "ChatMembervarKicked" then
redis:srem('GLOBLA:'..bot_id..'Groups_Users',group[i])  
q = q + 1
end
...
local LinkGroup = redis:get('GLOBLA:'..bot_id.."var:link:set:Group"..v) 
local WelcomeGroup = redis:get('GLOBLA:'..bot_id.."Get:Welcome:Group"..v) or ''
local var_Dev = redis:get('GLOBLA:'..bot_id.."Developer:Bot:Reply"..v) 
local var_Prt = redis:get('GLOBLA:'..bot_id.."President:User:Reply"..v) 
local var_Prx = redis:get('GLOBLA:'..bot_id.."Basic:User:Reply"..v) 
local var_Cto = redis:get('GLOBLA:'..bot_id.."Constructor:Group:Reply"..v) 
local var_Own = redis:get('GLOBLA:'..bot_id.."Manager:Group:Reply"..v) 
local var_Md = redis:get('GLOBLA:'..bot_id.."Admin:Group:Reply"..v) 
local var_Vip = redis:get('GLOBLA:'..bot_id.."Vip:Group:Reply"..v) 
local var_Mem = redis:get('GLOBLA:'..bot_id.."Mempar:Group:Reply"..v) 
if k == 1 then
Get_Json = Get_Json..'"'..v..'":{'
else
Get_Json = Get_Json..',"'..v..'":{'
...
if var_Dev then
Get_Json = Get_Json..'"var_Dev":"'..var_Dev..'",'
end
if var_Prt then
Get_Json = Get_Json..'"var_Prt":"'..var_Prt..'",'
end
if var_Cto then
Get_Json = Get_Json..'"var_Cto":"'..var_Cto..'",'
end
if var_Own then
Get_Json = Get_Json..'"var_Own":"'..var_Own..'",'
end
if var_Md then
Get_Json = Get_Json..'"var_Md":"'..var_Md..'",'
end
if var_Vip then
Get_Json = Get_Json..'"var_Vip":"'..var_Vip..'",'
end
if var_Mem then
Get_Json = Get_Json..'"var_Mem":"'..var_Mem..'",'
end
if LinkGroup then
...
File:write(Get_Json)
File:close()
sendDocument(msg.chat_id_, msg.id_,'./Offices/'..bot_id..'.json', '\n⌔︙تم جلب نسخه خاصه بالكروبات\n⌔︙يحتوي الملف على {'..#Groups..'} مجموعه')
elseif text == 'تفعيل تواصل البوت 🔔' then  
redis:del('GLOBLA:'..bot_id..'Lock:Twasl') 
send(msg.chat_id_, msg.id_,'⌔︙ تم تفعيل التواصل ') 
elseif text == 'تعطيل تواصل البوت 🔕' then  
redis:set('GLOBLA:'..bot_id..'Lock:Twasl',true) 
send(msg.chat_id_, msg.id_,'⌔︙ تم تعطيل التواصل ') 
end
end 
end
if TypeForChat == ("ForSuppur") then
if text ==  ""..Name_Bot..' شنو رئيك بهاذا' and tonumber(msg.reply_to_message_id_) > 0 and not redis:get('GLOBLA:'..bot_id.."var:Fun:Bots"..msg.chat_id_) then     
...
return false
elseif text == ""..Name_Bot..' شنو رئيك بهاي' and tonumber(msg.reply_to_message_id_) > 0 and not redis:get('GLOBLA:'..bot_id.."var:Fun:Bots"..msg.chat_id_) then    
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)},function(Arg, Data) 
...
if text == ("اضف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_GLOBLA(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @Spirta')   
return false
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, Functionvar, nil)
return false
end
if text == ("حذف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_GLOBLA(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @Spirta')   
return false
end
...
for k,v in pairs(list) do   
NAME = 'Tshake Chat'
ASAS = redis:smembers('GLOBLA:'..bot_id.."President:User"..v)
...
link = redis:get('GLOBLA:'..bot_id.."var:link:set:Group"..v) or ''
if k == 1 then
t = t..'"'..v..'":{"tek":"'..NAME..'",'
else
t = t..',"'..v..'":{"tek":"'..NAME..'",'
...
File:write(t)
File:close()
sendDocument(msg.chat_id_, msg.id_,'./Offices/'..bot_id..'.json', '📮┇ عدد مجموعات التي في البوت { '..#list..'}')
end
...
if da.var_.ID == "ChatMembervarCreator" then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @Spirta')   
return false
end
...
if not Constructor(msg) and redis:get('GLOBLA:'..bot_id.."var:Cheking:Seted"..msg.chat_id_) then 
...
if not Constructor(msg) and redis:get('GLOBLA:'..bot_id.."Lock:Ban:Group"..msg.chat_id_) then 
...
tdcli_function ({ ID = "ChangeChatMembervar", chat_id_ = msg.chat_id_, user_id_ = result.sender_user_id_, var_ = { ID = "ChatMembervarKicked" },},function(arg,data) 
...
tdcli_function ({ ID = "ChangeChatMembervar", chat_id_ = msg.chat_id_, user_id_ = result.sender_user_id_, var_ = { ID = "ChatMembervarLeft" },},function(arg,ban) end,nil)   
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, Functionvar, nil)
elseif text and text:match("^حظر عام @(.*)$") and Dev_GLOBLA(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @Spirta')   
return false
end
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^حظر عام @(.*)$")}, Functionvar, nil)
elseif text and text:match("^الغاء العام @(.*)$") and Dev_GLOBLA(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @Spirta')   
return false
end
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^رفع منشئ اساسي @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^تنزيل منشئ اساسي @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^رفع منشئ @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^تنزيل منشئ @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^رفع مدير @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^تنزيل مدير @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^رفع ادمن @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^تنزيل ادمن @(.*)$") }, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^رفع مميز @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^تنزيل مميز @(.*)$") }, Functionvar, nil)
...
function var_reply(extra, result, success)   
local varrt = redis:get('GLOBLA:'..bot_id.."Add:Validity:Group:Rt"..text:match("رفع (.*)")..msg.chat_id_)
if varrt == "مميز" and Admin(msg) then
redis:set('GLOBLA:'..bot_id.."Add:Validity:Users"..msg.chat_id_..result.sender_user_id_,text:match("رفع (.*)")) 
redis:sadd('GLOBLA:'..bot_id.."Vip:Group"..msg.chat_id_,result.sender_user_id_)  
elseif varrt == "ادمن" and Owner(msg) then 
redis:set('GLOBLA:'..bot_id.."Add:Validity:Users"..msg.chat_id_..result.sender_user_id_,text:match("رفع (.*)"))
redis:sadd('GLOBLA:'..bot_id.."Admin:Group"..msg.chat_id_,result.sender_user_id_)  
elseif varrt == "مدير" and Constructor(msg) then
redis:set('GLOBLA:'..bot_id.."Add:Validity:Users"..msg.chat_id_..result.sender_user_id_,text:match("رفع (.*)"))  
redis:sadd('GLOBLA:'..bot_id.."Manager:Group"..msg.chat_id_,result.sender_user_id_)  
elseif varrt == "عضو" and Admin(msg) then
redis:set('GLOBLA:'..bot_id.."Add:Validity:Users"..msg.chat_id_..result.sender_user_id_,text:match("رفع (.*)"))  
end
Send_Options(msg,result.sender_user_id_,"reply","⌔︙تم ترقيته : "..text:match("رفع (.*)").."")  
end   
tdcli_function ({ ID = "GetMessage", chat_id_ = msg.chat_id_, message_id_ = tonumber(msg.reply_to_message_id_) }, var_reply, nil)
end
...
function var_reply(extra, result, success)   
local varrt = redis:get('GLOBLA:'..bot_id.."Add:Validity:Group:Rt"..text:match("تنزيل (.*)")..msg.chat_id_)
if varrt == "مميز" and Admin(msg) then 
redis:srem('GLOBLA:'..bot_id.."Vip:Group"..msg.chat_id_,result.sender_user_id_)  
...
function var_username(extra, result, success)   
if (result.id_) then
local varrt = redis:get('GLOBLA:'..bot_id.."Add:Validity:Group:Rt"..text1[2]..msg.chat_id_)
if varrt == "مميز" and Admin(msg) then
redis:sadd('GLOBLA:'..bot_id.."Vip:Group"..msg.chat_id_,result.id_)  
...
tdcli_function ({ID = "SearchPublicChat",username_ = text1[3]},var_username,nil) 
end 
...
tdcli_function ({ ID = "ChangeChatMembervar", chat_id_ = msg.chat_id_, user_id_ = result.id_, var_ = { ID = "ChatMembervarKicked" },},function(arg,data) 
if (result and result.type_ and result.type_.ID == "ChannelChatInfo") then
send(msg.chat_id_,msg.id_,"⌔︙عذرا هاذا معرف قناة")   
return false 
end      
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^حظر @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ ID = "ChangeChatMembervar", chat_id_ = msg.chat_id_, user_id_ = result.id_, var_ = { ID = "ChatMembervarLeft" },},function(arg,ban) end,nil)   
Send_Options(msg,result.id_,"reply","⌔︙تم الغاء حظره من هنا")  
else
send(msg.chat_id_, msg.id_,"⌔︙المعرف خطأ ")
end
end
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^الغاء حظر @(.*)$") }, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^كتم @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^الغاء كتم @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^تقيد @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = TextEnd[4]}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^الغاء تقيد @(.*)$")}, Functionvar, nil)
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^طرد @(.*)$")}, Functionvar, nil)
elseif text and text:match("^حظر عام (%d+)$") and Dev_GLOBLA(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @Spirta')   
return false
end
...
elseif text and text:match("^الغاء العام (%d+)$") and Dev_GLOBLA(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @Spirta')   
return false
end
...
tdcli_function ({ ID = "ChangeChatMembervar", chat_id_ = msg.chat_id_, user_id_ = text:match("^حظر (%d+)$") , var_ = { ID = "ChatMembervarKicked" },},function(arg,data) 
if (data and data.code_ and data.code_ == 400 and data.message_ == "CHAT_ADMIN_REQUIRED") then 
send(msg.chat_id_, msg.id_,"⌔︙لا توجد لدي صلاحية حظر المستخدمين") 
return false  
end
...
tdcli_function ({ ID = "ChangeChatMembervar", chat_id_ = msg.chat_id_, user_id_ = text:match("^الغاء حظر (%d+)$") , var_ = { ID = "ChatMembervarLeft" },},function(arg,ban) end,nil)   
...
tdcli_function ({ ID = "ChangeChatMembervar", chat_id_ = msg.chat_id_, user_id_ = text:match("^طرد (%d+)$") , var_ = { ID = "ChatMembervarKicked" },},function(arg,data) 
if (data and data.code_ and data.code_ == 400 and data.message_ == "CHAT_ADMIN_REQUIRED") then 
send(msg.chat_id_, msg.id_,"⌔︙لا توجد لدي صلاحية حظر المستخدمين") 
return false  
end
...
redis:set('GLOBLA:'..bot_id.."Lock:kanser"..msg.chat_id_,true) 
Send_Options(msg,msg.sender_user_id_,"Close_var","⌔︙ تم قفل الكانسر ")
elseif text == "فتح الكانسر" and Owner(msg) then 
redis:del('GLOBLA:'..bot_id.."Lock:kanser"..msg.chat_id_) 
Send_Options(msg,msg.sender_user_id_,"Open_var","⌔︙تم فتح الكانسر ")
elseif text == "قفل الفارسيه" and msg.reply_to_message_id_ == 0 and Owner(msg) then 
redis:set('GLOBLA:'..bot_id.."Lock:Far"..msg.chat_id_,true) 
Send_Options(msg,msg.sender_user_id_,"Close_var","⌔︙تم قفـل الفارسيه")  
elseif text == "قفل الفشار" and msg.reply_to_message_id_ == 0 and Owner(msg) then 
redis:set('GLOBLA:'..bot_id.."Lock:Fsh"..msg.chat_id_,true) 
Send_Options(msg,msg.sender_user_id_,"Close_var","⌔︙تم قفـل الفشار")  
...
Send_Options(msg,msg.sender_user_id_,"Close_var","⌔︙تم قفـل الدردشه")  
...
Send_Options(msg,msg.sender_user_id_,"Close_var","⌔︙تم قفـل اضافة الاعضاء")  
...
Send_Options(msg,msg.sender_user_id_,"Close_var","⌔︙تم قفـل دخول الاعضاء")  
...
Send_Options(msg,msg.sender_user_id_,"Close_var","⌔︙تم قفـل البوتات")  
...
Send_Options(msg,msg.sender_user_id_,"Close_var_Kick","⌔︙تم قفـل البوتات")  
...
Send_Options(msg,msg.sender_user_id_,"Close_var","⌔︙تم قفـل الاشعارات")  
elseif text == "قفل التثبيت" and msg.reply_to_message_id_ == 0 and Constructor(msg) then 
redis:set('GLOBLA:'..bot_id.."var:lockpin"..msg.chat_id_, true) 
...
Send_Options(msg,msg.sender_user_id_,"Close_var","⌔︙تم قفـل التثبيت هنا")  
...
Send_Options(msg,msg.sender_user_id_,"Close_var","⌔︙تم قفـل تعديل")  
...
Send_Options(msg,msg.sender_user_id_,"Close_var","⌔︙تم قفـل جميع الاوامر")  
...
Send_Options(msg,msg.sender_user_id_,"Open_var","⌔︙تم فتح اضافة الاعضاء")  
elseif text == "فتح الفارسيه" and msg.reply_to_message_id_ == 0 and Owner(msg) then 
redis:del('GLOBLA:'..bot_id.."Lock:Far"..msg.chat_id_) 
Send_Options(msg,msg.sender_user_id_,"Open_var","⌔︙تم فتح الفارسيه")  
elseif text == "فتح الفشار" and msg.reply_to_message_id_ == 0 and Owner(msg) then 
redis:del('GLOBLA:'..bot_id.."Lock:Fsh"..msg.chat_id_) 
Send_Options(msg,msg.sender_user_id_,"Open_var","⌔︙تم فتح الفشار")  
elseif text == "فتح الدردشه" and msg.reply_to_message_id_ == 0 and Owner(msg) then 
redis:del('GLOBLA:'..bot_id.."Lock:text"..msg.chat_id_)  
Send_Options(msg,msg.sender_user_id_,"Open_var","⌔︙تم فتح الدردشه")  
elseif text == "فتح الدخول" and msg.reply_to_message_id_ == 0 and Admin(msg) then 
redis:del('GLOBLA:'..bot_id.."Lock:Join"..msg.chat_id_)  
Send_Options(msg,msg.sender_user_id_,"Open_var","⌔︙تم فتح دخول الاعضاء")  
...
Send_Options(msg,msg.sender_user_id_,"Open_var","⌔︙تم فتح البوتات")  
elseif text == "فتح البوتات " and msg.reply_to_message_id_ == 0 and Admin(msg) then 
redis:del('GLOBLA:'..bot_id.."Lock:Bot:kick"..msg.chat_id_)  
Send_Options(msg,msg.sender_user_id_,"Open_var","\n⌔︙تم فتح البوتات")  
...
Send_Options(msg,msg.sender_user_id_,"Open_var","⌔︙تم فتح الاشعارات")  
elseif text == "فتح التثبيت" and msg.reply_to_message_id_ == 0 and Constructor(msg) then 
redis:del('GLOBLA:'..bot_id.."var:lockpin"..msg.chat_id_)  
redis:srem('GLOBLA:'..bot_id.."Lock:pin",msg.chat_id_)
Send_Options(msg,msg.sender_user_id_,"Open_var","⌔︙تم فتح التثبيت هنا")  
...
Send_Options(msg,msg.sender_user_id_,"Open_var","⌔︙تم فتح تعديل")  
...
redis:set('GLOBLA:'..bot_id.."var:Fun:Group"..msg.chat_id_,"true")
elseif text == "تفعيل اوامر التحشيش" and Owner(msg) then    
send(msg.chat_id_, msg.id_,'⌔︙تم تفعيل اوامر التحشيش')
redis:del('GLOBLA:'..bot_id.."var:Fun:Group"..msg.chat_id_)
...
redis:del('GLOBLA:'..bot_id..'Lock:Id:Py:Photo'..msg.chat_id_) 
...
redis:del('GLOBLA:'..bot_id.."Lock:Game:Group"..msg.chat_id_) 
send(msg.chat_id_, msg.id_,"⌔︙تم تعطيل الالعاب") 
elseif text == "تفعيل الالعاب" and Owner(msg) then  
redis:set('GLOBLA:'..bot_id.."Lock:Game:Group"..msg.chat_id_,true) 
...
redis:del('GLOBLA:'..bot_id.."Lock:Ban:Group"..msg.chat_id_)
...
redis:del('GLOBLA:'..bot_id.."var:Cheking:Seted"..msg.chat_id_)
...
num = num + 1  
tdcli_function ({ ID = "ChangeChatMembervar", chat_id_ = msg.chat_id_, user_id_ = y.user_id_, var_ = { ID = "ChatMembervarLeft"}, }, dl_cb, nil)  
end  
...
local c = 0
for i=0 , #admins do 
if tah.members_[i].var_.ID == "ChatMembervarEditor" then  
x = x + 1 
end
...
redis:del('GLOBLA:'..bot_id.."var:link:set:Group"..msg.chat_id_) 
...
redis:setex('GLOBLA:'..bot_id.."var:link:set"..msg.chat_id_..""..msg.sender_user_id_,120,true) 
...
elseif text == 'روابط الكروبات' and Dev_GLOBLA(msg) then
local list = redis:smembers('GLOBLA:'..bot_id..'Groups_Users') 
test = '⌔︙روابط الكروبات \n\n'
for k,v in pairs(list) do 
local link = redis:get('GLOBLA:'..bot_id.."var:link:set:Group"..v)   
...
local var_Link = redis:get('GLOBLA:'..bot_id.."Link_Group"..msg.chat_id_)
if var_Link then
send(msg.chat_id_, msg.id_,"⌔︙جلب الرابط معطل") 
return false  
end
local link = redis:get('GLOBLA:'..bot_id.."var:link:set:Group"..msg.chat_id_)            
if link then                              
...
send(msg.chat_id_,msg.id_,"- Link group :\n••━━━━━━━━━━••\n ["..linkgpp.result.."]")
redis:set('GLOBLA:'..bot_id.."var:link:set:Group"..msg.chat_id_,linkgpp.result)
else
...
if result.members_[i].var_.ID == "ChatMembervarMember" then  
tr = ""
elseif result.members_[i].var_.ID == "ChatMembervarEditor" then  
t = t + 1
tr = " {★}"
end
...
print('&&&')
function Functionvar(extra, result, success)
if (result.id_) then
if Dev_GLOBLA_User(result.id_) == true then
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^تنزيل الكل @(.*)$")}, Functionvar, nil)
end
if text == ("تنزيل الكل") and msg.reply_to_message_id_ ~= 0 and Owner(msg) then
function Function_var(extra, result, success)
if Dev_GLOBLA_User(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_,"⌔︙لا تستطيع تنزيل المطور الاساسي")
return false 
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, Function_var, nil)
...
if DeveloperBot(msg) and not redis:get('GLOBLA:'..bot_id.."Lock:Left"..msg.chat_id_) then 
tdcli_function ({ID = "ChangeChatMembervar",chat_id_=msg.chat_id_,user_id_=bot_id,var_={ID = "ChatMembervarLeft"},},function(e,g) end, nil) 
...
if DeveloperBot(msg) and not redis:get('GLOBLA:'..bot_id.."Lock:Left"..msg.chat_id_) then 
tdcli_function ({ID = "ChangeChatMembervar",chat_id_=GP_ID[2],user_id_=bot_id,var_={ID = "ChatMembervarLeft"},},function(e,g) end, nil) 
send(msg.chat_id_, msg.id_,"⌔︙تم حبيبي حغادر") 
...
if redis:get('GLOBLA:'..bot_id.."var:Broadcasting:Bot") and not Dev_GLOBLA(msg) then 
...
if redis:get('GLOBLA:'..bot_id.."var:lockpin"..msg.chat_id_) then    
lock_pin = "{✔️}"
else 
lock_pin = "{✖}"    
end
...
if not redis:get('GLOBLA:'..bot_id.."var:Reply:Manager"..msg.chat_id_) then
ReplyManager = "← {✔️}"
else
ReplyManager = "← {✖}"
end
if not redis:get('GLOBLA:'..bot_id.."var:Reply:Sudo"..msg.chat_id_) then
ReplySudo = "← {✔️}"
else
ReplySudo = "← {✖}"
end
...
if not redis:get('GLOBLA:'..bot_id.."Lock:Id:Py:Photo"..msg.chat_id_) then
IdPyPhoto = "← {✔️}"
else
IdPyPhoto = "← {✖}"
end
if not redis:get('GLOBLA:'..bot_id.."var:Cheking:Kick:Me:Group"..msg.chat_id_)  then
KickMe = "← {✔️}"
else
KickMe = "← {✖}"
end
if not redis:get('GLOBLA:'..bot_id.."Lock:Ban:Group"..msg.chat_id_)  then
Banusers = "← {✔️}"
else
Banusers = "← {✖}"
end
if not redis:get('GLOBLA:'..bot_id.."var:Cheking:Seted"..msg.chat_id_) then
Setusers = "← {✔️}"
else
Setusers = "← {✖}"
end
...
if not redis:get('GLOBLA:'..bot_id.."var:Fun:Group"..msg.chat_id_) then
FunGroup = "← {✔️}"
else
FunGroup = "← {✖}"
end
...
if text == 'كشف' and tonumber(msg.reply_to_message_id_) > 0 and not redis:get('GLOBLA:'..bot_id..'Lock:Id:Photo'..msg.chat_id_) then
function Function_var(extra, result, success)
...
local var_Gps = Get_Rank(Id,msg.chat_id_)
send(msg.chat_id_, msg.id_,'\n*⌔︙الاسم ← ('..DevZain..')\n⌔︙الايدي ← '..Id..'\n⌔︙المعرف ← *['..UserName_User..']*\n⌔︙الرتبة ← '..var_Gps..'\n⌔︙نوع الكشف ←بالرد*') 
end,nil)   
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, Function_var, nil)
return false
elseif text and text:match("^كشف @(.*)$") and not redis:get('GLOBLA:'..bot_id..'Lock:Id:Photo'..msg.chat_id_) then
...
tdcli_function ({ID = "SearchPublicChat",username_ = username}, Function_var, nil)
return false
end
if text == 'ايدي' and tonumber(msg.reply_to_message_id_) == 0 and not redis:get('GLOBLA:'..bot_id..'Lock:Id:Photo'..msg.chat_id_) then
...
local Get_Is_Id = Get_Is_Id:gsub('#stast',var_Gps) 
...
elseif text == 'ايدي' and tonumber(msg.reply_to_message_id_) > 0 and not redis:get('GLOBLA:'..bot_id..'Lock:Id:Photo'..msg.chat_id_) then
function Function_var(extra, result, success)
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
if data.first_name_ == false then
send(msg.chat_id_, msg.id_,'⌔︙ الحساب محذوف لا توجد معلوماته ')
...
local Num_Games = redis:get('GLOBLA:'..bot_id.."var:Msg_User"..msg.chat_id_..":"..data.id_) or 0
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, Function_var, nil)
return false
elseif text and text:match("^ايدي @(.*)$") and not redis:get('GLOBLA:'..bot_id..'Lock:Id:Photo'..msg.chat_id_) then
...
if redis:get('GLOBLA:'..bot_id.."Lock:Game:Group"..msg.chat_id_) then
redis:del('GLOBLA:'..bot_id.."var:Set:Sma"..msg.chat_id_)
Random = {"🍏","🍎","🍐","🍊","🍋","🍉","🍇","🍓","🍈","🍒","🍑","🍍","🥥","🥝","🍅","🍆","🥑","🥦","🥒","🌶","🌽","🥕","🥔","🥖","🥐","🍞","🥨","🍟","🧀","🥚","🍳","🥓","🥩","🍗","🍖","🌭","🍔","🍠","🍕","🥪","🥙","☕️","??","🥤","🍶","🍺","🍻","🏀","⚽️","🏈","⚾️","🎾","🏐","🏉","🎱","🏓","🏸","🥅","🎰","🎮","🎳","🎯","🎲","🎻","🎸","🎺","🥁","🎹","🎼","🎧","🎤","🎬","🎨","🎭","🎪","🎟","🎫","🎗","🏵","🎖","🏆","🥌","🛷","🚗","🚌","🏎","🚓","🚑","🚚","🚛","🚜","🇮🇶","⚔","🛡","🔮","🌡","💣","📌","📍","📓","📗","📂","📅","📪","📫","📬","📭","⏰","📺","🎚","☎️","📡"}
SM = Random[math.random(#Random)]
redis:set('GLOBLA:'..bot_id.."var:Random:Sm"..msg.chat_id_,SM)
send(msg.chat_id_, msg.id_,"⌔︙اسرع واحد يدز هاذا السمايل ? ~ {`"..SM.."`}")
return false
end
elseif text == "الاسرع" or tect == "ترتيب" then
if redis:get('GLOBLA:'..bot_id.."Lock:Game:Group"..msg.chat_id_) then
redis:del('GLOBLA:'..bot_id.."var:Speed:Tr"..msg.chat_id_)
KlamSpeed = {"سحور","سياره","استقبال","قنفه","ايفون","بزونه","مطبخ","كرستيانو","دجاجه","مدرسه","الوان","غرفه","ثلاجه","كهوه","سفينه","العراق","محطه","طياره","رادار","منزل","مستشفى","كهرباء","تفاحه","اخطبوط","سلمون","فرنسا","برتقاله","تفاح","مطرقه","بتيته","لهانه","شباك","باص","سمكه","ذباب","تلفاز","حاسوب","انترنيت","ساحه","جسر"};
name = KlamSpeed[math.random(#KlamSpeed)]
redis:set('GLOBLA:'..bot_id.."var:Klam:Speed"..msg.chat_id_,name)
...
if redis:get('GLOBLA:'..bot_id.."Lock:Game:Group"..msg.chat_id_) then
redis:del('GLOBLA:'..bot_id.."var:Set:Hzora"..msg.chat_id_)
...
redis:set('GLOBLA:'..bot_id.."var:Klam:Hzor"..msg.chat_id_,name)
...
if redis:get('GLOBLA:'..bot_id.."Lock:Game:Group"..msg.chat_id_) then
redis:del('GLOBLA:'..bot_id.."var:Set:Maany"..msg.chat_id_)
Maany_Rand = {"قرد","دجاجه","بطريق","ضفدع","بومه","نحله","ديك","جمل","بقره","دولفين","تمساح","قرش","نمر","اخطبوط","سمكه","خفاش","اسد","فأر","ذئب","فراشه","عقرب","زرافه","قنفذ","تفاحه","باذنجان"}
name = Maany_Rand[math.random(#Maany_Rand)]
redis:set('GLOBLA:'..bot_id.."var:Maany"..msg.chat_id_,name)
...
if redis:get('GLOBLA:'..bot_id.."Lock:Game:Group"..msg.chat_id_) then
redis:del('GLOBLA:'..bot_id.."var:Set:Aks"..msg.chat_id_)
katu = {"باي","فهمت","موزين","اسمعك","احبك","موحلو","نضيف","حاره","ناصي","جوه","سريع","ونسه","طويل","سمين","ضعيف","شريف","شجاع","رحت","عدل","نشيط","شبعان","موعطشان","خوش ولد","اني","هادئ"}
name = katu[math.random(#katu)]
redis:set('GLOBLA:'..bot_id.."var:Set:Aks:Game"..msg.chat_id_,name)
...
redis:set('GLOBLA:'..bot_id.."var:GAMES:NUM"..msg.chat_id_,Num) 
send(msg.chat_id_, msg.id_,"\n⌔︙اهلا بك عزيزي في لعبة التخمين :\nٴ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n".."⌔︙ملاحظه لديك { 3 } محاولات فقط فكر قبل ارسال تخمينك \n\n".."⌔︙سيتم تخمين عدد ما بين ال {1 و 20} اذا تعتقد انك تستطيع الفوز جرب واللعب الان ؟ ")
redis:setex('GLOBLA:'..bot_id.."var:GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
elseif text == "محيبس" or text == "بات" then
if redis:get('GLOBLA:'..bot_id.."Lock:Game:Group"..msg.chat_id_) then   
Num = math.random(1,6)
redis:set('GLOBLA:'..bot_id.."var:Games:Bat"..msg.chat_id_,Num) 
send(msg.chat_id_, msg.id_,[[
*➀       ➁     ➂      ➃      ➄     ➅
↓      ↓     ↓      ↓     ↓     ↓
👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊
...
redis:setex('GLOBLA:'..bot_id.."var:SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
...
redis:del('GLOBLA:'..bot_id.."var:Set:Moktlf:Bot"..msg.chat_id_)
redis:set('GLOBLA:'..bot_id.."var::Set:Moktlf"..msg.chat_id_,name)
...
redis:set('GLOBLA:'..bot_id.."var:Set:Amth"..msg.chat_id_,name)
redis:del('GLOBLA:'..bot_id.."var:Set:Amth:Bot"..msg.chat_id_)
...
redis:set('GLOBLA:'..bot_id.."var:id:user"..msg.chat_id_,zain)  
redis:setex('GLOBLA:'..bot_id.."var:Add:msg:user" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 120, true)  
...
redis:set('GLOBLA:'..bot_id.."var:idgem:user"..msg.chat_id_,zain)  
redis:setex('GLOBLA:'..bot_id.."var:gemadd:user" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 120, true)  
...
elseif text and text:match("^اضف رسائل (%d+)$") and msg.reply_to_message_id_ ~= 0 and Constructor(msg) then
function reply(extra, result, success)
redis:del('GLOBLA:'..bot_id.."var:Msg_User"..msg.chat_id_..":"..result.sender_user_id_) 
...
tdcli_function({ID="ChangeChatMembervar",chat_id_=msg.chat_id_,user_id_=msg.sender_user_id_,var_={ID="ChatMembervarKicked"},},function(arg,data) 
if (data and data.code_ and data.code_ == 400 and data.message_ == "CHAT_ADMIN_REQUIRED") then 
...
tdcli_function ({ ID = "ChangeChatMembervar", chat_id_ = msg.chat_id_, user_id_ = msg.sender_user_id_, var_ = { ID = "ChatMembervarLeft" },},function(arg,ban) end,nil)   
return false
end
end,nil)   
else
...
for i=0 , #admins do
if data.members_[i].bot_info_ == false and data.members_[i].var_.ID == "ChatMembervarEditor" then
...
local admins = data.members_
for i=0 , #admins do
if data.members_[i].var_.ID == "ChatMembervarCreator" then
...
elseif text and text:match("^تعين عدد الاعضاء (%d+)$") and Dev_GLOBLA(msg) then
redis:set('GLOBLA:'..bot_id..'Num:Add:Bot',text:match("تعين عدد الاعضاء (%d+)$") ) 
send(msg.chat_id_, msg.id_,'*⌔︙ تم تعيين عدد اعضاء تفعيل البوت اكثر من : '..text:match("تعين عدد الاعضاء (%d+)$")..' عضو *')
...
send(msg.chat_id_, msg.id_,'['..SudoUserName..']')
end
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(redis:get('GLOBLA:'..bot_id..'Num:Add:Bot') or 0) and not Dev_GLOBLA(msg) then
...
sendText(Developer,'⌔︙تم تفعيل مجموعه جديده\n'..'\n⌔︙بواسطة : '..Name..''..'\n⌔︙ايدي المجموعه : `'..IdChat..'`'..'\n⌔︙عدد اعضاء المجموعه *: '..NumMember..'*'..'\n⌔︙اسم المجموعه : ['..NameChat..']'..'\n⌔︙الرابط : ['..LinkGp..']',0,'md')
end
end
end,nil) 
end,nil) 
...
if not Dev_GLOBLA(msg) then
sendText(Developer,'⌔︙تم تعطيل مجموعه جديده\n'..'\n⌔︙بواسطة : '..Name..''..'\n⌔︙ايدي المجموعه : `'..IdChat..'`\n⌔︙اسم المجموعه : ['..NameChat..']',0,'md')
end
end
end,nil) 
end,nil) 
...
if DataChat and DataChat.var_.ID == "ChatMembervarEditor" or DataChat and DataChat.var_.ID == "ChatMembervarCreator" then
if DataChat and DataChat.user_id_ == msg.sender_user_id_ then
if DataChat.var_.ID == "ChatMembervarCreator" then
var_Rt = 'المالك'
elseif DataChat.var_.ID == "ChatMembervarEditor" then
var_Rt = 'مشرف'
else 
var_Rt = 'عضو'
end
...
if data.channel_.var_.ID == ("ChatMembervarKicked") then 
...
tdcli_function ({ID = "GetMessages",chat_id_ = msg.chat_id_,message_ids_ = Msgs},function(arg,data);MsgsDel = {};for i=0 ,data.total_count_ do;if not data.messages_[i] then;if not MsgsDel[0] then;MsgsDel[0] = Msgs[i];end;table.insert(MsgsDel,Msgs[i]);end;end;if MsgsDel[0] then;tdcli_function({ID="DeleteMessages",chat_id_ = arg.chat_id_,message_ids_=MsgsDel},function(arg,data)end,nil);end;end,{chat_id_=msg.chat_id_}) tdcli_function({ID = "GetChannelMembers",channel_id_ = msg.chat_id_:gsub("-100",""),filter_ = {ID = "ChannelMembersBots"},offset_ = 0,limit_ = 100 },function(arg,tah) local admins = tah.members_ for i=0 , #admins do if tah.members_[i].var_.ID ~= "ChatMembervarEditor" and not is_Admin(msg) then tdcli_function ({ID = "ChangeChatMembervar",chat_id_ = msg.chat_id_,user_id_ = admins[i].user_id_,var_ = {ID = "ChatMembervarKicked"},}, function(arg,f) end, nil) end end end,nil)  
end
end     
end
end
...
if redis:get('GLOBLA:'..bot_id.."Lock:edit"..result.chat_id_) and not textedit and not PresidentGroup(result) then
